...
public class CreditCardEditText extends AppCompatEditText {
    ...
    private final int mDefaultDrawableResId = R.drawable.ic_credit_card; //default credit card image
    ...
    private void init() {
        if (mCCPatterns == null) {
            ...
            mCCPatterns.put(R.drawable.ic_visa, Pattern.compile("^4[0-9]{2,12}(?:[0-9]{3})?$"));
            mCCPatterns.put(R.drawable.ic_mastercard, Pattern.compile("^5[1-5][0-9]{1,14}$"));
    ...
    protected void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
        ...
        for (int i = 0; i < mCCPatterns.size(); i++) {
            ...
            if (m.find()) {
                mDrawableResId = key;
                setTag(mCCPatterns.get(key).toString());
                break;
            }
        }
